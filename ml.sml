...
fun solve (typ1 ~ typ2) =
    (case (typ1, typ2)
            of (TYVAR alpha, typ) =>
              (case typ
                of CONAPP(t1, t1s) => 
                    if member alpha (freetyvars typ)
                      then unsatisfiableEquality(TYVAR alpha, CONAPP (t1, t1s))
                    else if eqType(typ1, t1)
                      then idsubst
                    else
                      alpha |--> (CONAPP(t1, t1s))
                | (TYVAR alpha2) => if eqType(typ1, typ)
                                      then idsubst
                                    else alpha |--> typ
                | _ => alpha |--> typ)
            | (typ1, TYVAR(beta)) => solve (TYVAR beta ~ typ1)
            | (TYCON(constr1), TYCON(constr2)) =>
                if eqType(TYCON constr1, TYCON constr2)
                  then idsubst
                else unsatisfiableEquality(TYCON constr1, TYCON constr2)
            | (CONAPP (t1, t1s), CONAPP(t2, t2s)) =>
                let
                  val cnstrnts = (ListPair.mapEq (fn (typ1, typ2) =>
                    typ1 ~ typ2) (t1::t1s, t2::t2s)) handle UnequalLengths =>
                      unsatisfiableEquality (CONAPP(t1, t1s), CONAPP(t2, t2s))
                in
                  solve (conjoinConstraints cnstrnts)
                end
            | _ => unsatisfiableEquality(typ1, typ2))

    | solve (constr1 /\ constr2) =
        let
...
fun isIdempotent pairs =
    let fun distinct a' (a, tau) = a <> a' andalso not (member a'
                                    (freetyvars tau))
...
val () = Unit.checkAssert "tyvar can be solved"
        (fn () => hasGoodSolution (TYVAR "'a" ~ TYVAR "'b"))

val () = Unit.checkAssert "tycon can be solved"
        (fn () => hasGoodSolution (TYCON "'a" ~ TYCON "'b"))
                                       
val () = Unit.checkAssert "tyvar/tycon can be solved"
        (fn () => hasGoodSolution (TYVAR "'a" ~ TYCON "'b"))

val () = Unit.checkAssert "tycon/tyvar can be solved"
        (fn () => hasGoodSolution (TYCON "'a" ~ TYVAR "'b"))

val () = Unit.checkAssert "tyvar/conapp can be solved"
        (fn () => hasGoodSolution (TYCON "'a" ~ 
                    CONAPP (TYCON "list", [TYCON "int"])))

val () = Unit.checkAssert "CONAPP/int list ~int  cannot be solved"
        (fn () => hasNoSolution (CONAPP (TYCON "list", [inttype]) ~ inttype))
        
val () = Unit.checkAssert "CONAPP/int ~int list  cannot be solved"
...
(* function [[literal]], to infer the type of a literal constant ((prototype)) 438b *)
      fun literal (SYM (a))   = (symtype,   TRIVIAL)
        | literal (NUM (a))   = (inttype,   TRIVIAL)
        | literal (NIL )      = (listtype (freshtyvar()),  TRIVIAL)
        | literal (BOOLV (a)) = (booltype,  TRIVIAL)
        | literal (PAIR (a, b)) =
          let
            val (ta, ca) = literal a
            val (tb, cb) = literal b
            val constr   = ca /\ cb /\ tb ~ listtype ta
          in
            (listtype ta, constr)
          end
        | literal _ = raise BugInTypeInference "NOT LITERAL"
...
(* function [[ty]], to infer the type of a \nml\ expression, given [[Gamma]] 438c *)
      fun ty (LITERAL n) = literal n
        ...
        | ty (IFX (e1, e2, e3))        =
              let val (typ1, constr1)  = ty e1
                  val (typ2, constr2)  = ty e2
                  val (typ3, constr3)  = ty e3
              in (typ2, constr1 /\ constr2 /\ constr3 /\ (typ1 ~ booltype) /\
                  (typ2 ~ typ3))
              end

        | ty (BEGIN es)                =
              (case typesof(es, Gamma)
                of (typs, constr)      => (List.last typs, constr)
                                      handle List.Empty => (unittype, TRIVIAL))
                                      
        | ty (LAMBDA (formals, body))  =
              let
                val mapFreshtys        = map freshtyvar formals
                val extGamma           = ListPair.foldr (fn (name, alpha, gamma)
                    => bindtyscheme (name, FORALL ([], alpha), gamma))
                      Gamma (formals, mapFreshtys)
                val (typ, constr)      = typeof(body, extGamma)
                val typ_func           = funtype(mapFreshtys, typ)
              in (typ_func, constr)
              end

        | ty (LETX (LET, bs, body))    =
              let
                val (ys, es)           = ListPair.unzip bs
                val (typs, constr)     = typesof (es, Gamma)
                val theta1             = solve constr
                val alphas             = inter(dom theta1, freetyvarsGamma
                                                Gamma)
                val constrlist         = map (fn (alpha) =>
                      TYVAR alpha ~ (varsubst theta1 alpha)) alphas
                val consconjoined      = conjoinConstraints constrlist
                val sigmalist          = map (fn (typ) =>
                      generalize (tysubst theta1 typ,
                      union (freetyvarsGamma Gamma,
                      freetyvarsConstraint consconjoined))) typs
                val newEnvironment     = ListPair.foldrEq bindtyscheme Gamma
                      (ys, sigmalist)
                val (tbody, cbody)     = typeof (body, newEnvironment)
              in
                (tbody, consconjoined /\ cbody)
              end
                    
        | ty (LETX (LETREC, bs, body)) =
            let
              val (ys, es)             = ListPair.unzip bs
              val checkLambdas         = List.all (fn (LAMBDA (ys, exp)) => true
                                          | e => raise TypeError(expString e ^
                                                  " is not a lambda")) es
              val freshtys             = map freshtyvar ys
              val newEnvironment       = foldr (fn ((name, alpha), gamma) =>
                    bindtyscheme (name, FORALL ([], alpha), gamma))
                    Gamma (ListPair.zipEq(ys, freshtys))
              val (typs, constrs)      = typesof(es, newEnvironment)
              val constrlist           = constrs :: (map (fn ((typs, alpha)) => 
                                                    typs ~ alpha)
                                                    (ListPair.zipEq(typs,
                                                    freshtys)))
              val consconjoined   = conjoinConstraints constrlist
              val constr          = constrs /\ consconjoined
              val theta           = solve constr
              val alphas          = inter (dom theta, freetyvarsGamma Gamma)
              val constrlist2     = map (fn (a) => TYVAR a ~ (tysubst theta
                                                (TYVAR a))) alphas
              val consconjoined2  = conjoinConstraints constrlist2
              val sigmas        = map (fn(typ) => generalize (tysubst
                                        theta typ, union(freetyvarsGamma Gamma,
                                        freetyvarsConstraint consconjoined2)))
                                        typs
              val newEnvironment2 = ListPair.foldrEq bindtyscheme
                                      Gamma(ys, sigmas)
              val (tbody, cbody)  = typeof (body, newEnvironment2)
            in
              (tbody, consconjoined /\ cbody)
            end
